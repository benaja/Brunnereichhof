<template>
  <fragment>
    ...
    ></navigation-bar>
    <div>
      <h1 class="text-center mt-4 mb-8 display-1">
        Woche {{ formatedWeek }}
      </h1>
      <v-row class="mx-1">
        <v-col
      ...
      >
        ...
        <comment
          grid-area="comment-0"
          :text="rapport.comment_mo"
        />
        <comment
          grid-area="comment-1"
          :text="rapport.comment_tu"
        />
        <comment
          grid-area="comment-2"
          :text="rapport.comment_we"
        />
        <comment
          grid-area="comment-3"
          :text="rapport.comment_th"
        />
        <comment
          grid-area="comment-4"
          :text="rapport.comment_fr"
        />
        <comment
          grid-area="comment-5"
          :text="rapport.comment_sa"
        />
        <template v-for="(rapportdetailsByEmployee) of rapport.rapportdetails">
          ...
          >
            {{ rapportdetailsByEmployee[0].employee.lastname }}
            {{ rapportdetailsByEmployee[0].employee.firstname }}
          </p>
          <template v-for="(rapportdetail, index) of rapportdetailsByEmployee">
            ...
            >
              ...
              <p
                v-if="settings.rapportFoodTypeEnabled"
              >
                {{ foodTypes.find(f => f.value === rapportdetail.foodtype_id).text }}
              </p>
              <p class="overline mb-0">
                Kultur
              </p>
...
export default {
  ...
  computed: {
    ...mapGetters(['settings']),
    formatedWeek() {
      const date = this.$moment(this.rapport.startdate, 'YYYY-MM-DD', 'de-ch')
    ...
    gridLayout() {
      if (this.$vuetify.breakpoint.smAndDown) {
        const gridTemplateColumns = '100%'
        let gridTemplateRows = ''
        let gridTemplateAreas = ''
        for (let i = 0; i < 6; i++) {
          gridTemplateAreas += `"weekday-${i}" "comment-${i}"`
          gridTemplateRows += 'auto auto'
          for (const rapportdetailsByEmployee of this.rapport.rapportdetails) {
  ...
  mounted() {
    this.isLoading = true
    Promise.all([
      this.axios.get(`/rapports/${this.id}`).then(response => {
