      ...
      <v-card max-width="600px">
        ...
        <v-card-text>
          ...
          >
            ...
            <v-date-picker v-model="date" no-title scrollable first-day-of-week="1" locale="ch-de">
              <v-spacer></v-spacer>
              <v-btn text color="primary" @click="datePickerMenu = false">Abbrechen</v-btn>
              <v-btn text color="primary" @click="$refs.dateMenu.save(date)">OK</v-btn>
            </v-date-picker>
          </v-menu>
          <v-divider></v-divider>
          <div v-for="dayTotal of dayTotals" :key="dayTotal.employee.id">
            <p class="my-2">
              <span
