...
{
    private $monthNames = ["Januar", "Februar", "März", "April", "Mai", "Juni",
    ...
    public function generateEmployeeList(Request $request){
        $request->user()->authorizeRoles(['admin']);
    ...
    // GET overview/worker/month/{month}
    public function workerMonthRapport(Request $request, $month){
        ...
        $name = explode(" ", $request->name);
        $worker = User::where([['firstname', '=', $name[0]], ['lastname', '=', $name[count($name) - 1]], ['authorization_id', AuthorizationType::Worker]])->first();
    ...
    // GET overview/employee/year/{year}
    public function generateEmployeYearOverview(Request $request, $year){
        $request->user()->authorizeRoles(['admin']);

        $name = explode(" ", $request->name);
        $employee = Employee::where([['firstname', '=', $name[0]], ['lastname', '=', $name[count($name)-1]]])->first();
